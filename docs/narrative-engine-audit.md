...
| Medium | Register rules conflict: fragments are permitted in one section, complete sentence titles and deck-wide titles-only tests are mandatory elsewhere. | Scope sentence/title-chain rules to Boardroom and use beat + written narration for Keynote. |
...
- Final automated and rendering results are recorded in the accompanying validation note. A baseline font-fetch failure is reported separately from passing checks; it is not hidden by disabling the failing test.

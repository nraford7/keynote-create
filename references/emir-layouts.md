...
**Source of truth:** Always read the local `deck.template.html` for the exact HTML scaffolding. This file is the picker.

## The 19 layouts

| # | Layout | Theme | Use when |
|---|---|---|---|
| 1 | COVER | dark | First slide of every deck. Wordmark + subtitle + meta block over the forge gradient. |
| 2 | SECTION DIVIDER | dark | Transition between major acts. Watermark numeral, single phrase, gold rule. |
...
| 7 | STATS | light | 2-3 standout numbers, each with one-line label. `.stat` blocks with gold top rule. |
| 8 | NUMBERED LIST | light | 4+ ordered steps (playbook, sequence). Large gold numerals, single-sentence steps. |
| 9 | PULL QUOTE / OBSERVATION | dark | An italic insight set apart by gold-faded rules. Title doubles as quote. |
| 10 | FORGE IMAGE (full-bleed) | dark | A single image moment. Image breathes; title and caption sit left under the gradient. |
| 11 | CLOSING | dark | Last slide. EMIR wordmark, title-rule, punchline in `h-gold`, contact line. |
...
The EMIR template alternates. Don't put more than 3 light slides in a row, or more than 2 dark slides in a row. Use dark layouts (VERDICT, PULL QUOTE, FORGE, SECTION DIVIDER, HALF-BLEED) as punctuation between light editorial slides.
...
| Image cue in source (`[image: forge floor]`, `Photo of...`) | SPLIT / HALF-BLEED / FORGE / FIGURE CARDS |
...
2. **Read** the corresponding template slide markup in the local `deck.template.html`.
3. **Rewrite** the slide's `<div class="slide-wrap"><div class="slide ...">…</div></div>` block in place, preserving the footer's brand + pageno.
4. **Validate** div balance after every rewrite — the EMIR docs warn that imbalanced divs break the deck silently. After all rewrites, count `<div` and `</div>` in the file; they must match.
...
The EMIR template uses `h3` (46px serif weight 400) for some layouts (cards at slide 5, stats at slide 7) by convention. **Override that during promotion.** In the narrative-title model the title carries the story, so it must stay visually dominant.
...
- **Drop eyebrows by default.** Eyebrows render as tracked-uppercase 13px gold-dark text. They compete with the title for the eye, especially with `text-transform:uppercase` making them look shouty. The EMIR template uses eyebrows because the template's slides need section context — promoted narrative slides don't. Omit unless the eyebrow is acting as a quiet act-label breadcrumb (e.g. "Act 1 · Context") and the deck has clearly demarcated acts.
...
- **Putting body text in gold.** `--gold-dark` and `--gold` are heading and accent colours only. Body type fails WCAG AA in gold.
...
When the deck frontmatter carries `mode: keynote`, the render script ignores the 19 EMIR layouts and routes every slide through this image-led family instead. The visual grammar is a **full-bleed image with a hard-edged caption box anchored top-left**, and a **one-line subcaption beneath it** — the house style of the corpus decks. There is no light/dark EMIR alternation and no gold-serif chrome; the images carry the rhythm. A top scrim darkens the upper third so the caption and subcaption read over any image.
...
**Footer.** Keynote slides carry a single small page number, lower-right, low-opacity white — not the EMIR brand footer. Wordless and one-word slides keep it too (it stays out of the way).
